						...
						<div class="col">
							...
							<?php foreach ($tarefa as $key => $value) { ?>
								...
								<div class="row mb-3 d-flex align-items-center tarefa">
									<div id="tarefa_<?= $value->id ?>" class="col-sm-9"><?= $value->tarefa ?> </div>
									<div class="col-sm-3 mt-2 d-flex justify-content-between">
										<i class="fas fa-trash-alt fa-lg text-danger" onclick="excluir_pedentes('<?= $value->id ?>')"></i>

										<?php if($value->id_status == 1) { ?>
										<i class="fas fa-edit fa-lg text-info" onclick="editar_pedentes('<?= $value->id ?>' , '<?= $value->tarefa ?>')"></i>
										<i class="fas fa-check-square fa-lg text-success" onclick="marcar_pedentes(<?= $value->id ?>)"></i>
